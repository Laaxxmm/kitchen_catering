...
/**
 * Live countdown to the order's event time. Colour-codes urgency so the
 * cook works the soonest deadlines first:
 *   overdue / < 30 min  → red
 *   < 2 h               → amber
 *   otherwise           → calm grey
...
function Countdown({ eventDate }: { eventDate: string }) {
  ...
  const diff = new Date(eventDate).getTime() - now;
    ...
    >
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <Countdown eventDate={order.eventDate} />
        </div>
      </div>
