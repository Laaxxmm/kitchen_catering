...
import { approvePurchaseRequisition, rejectPurchaseRequisition } from "@/server/actions/purchase-requisitions";
...
/**
 * Manager's approvals in one place — chef-proposed order changes, stock
 * requests, and purchase orders — each with approve/reject inline. No
 * drilling into detail pages to sign off routine items.
 */
export function ManagerApprovalsBoard({ orderChanges, stockRequests, purchaseOrders }: Props) {
  const total = orderChanges.length + stockRequests.length + purchaseOrders.length;
  if (total === 0) return null;

  const tabs = [
    { key: "orders", label: "Order changes", hint: "Chef suggestions", count: orderChanges.length },
    { key: "stock", label: "Stock requests", hint: "Approve to buy", count: stockRequests.length },
      ...
      <WorkTabs tabs={tabs} emptyHint="Nothing in {tab} right now.">
        {(active) => {
          ...
          if (active === "stock") {
            return (
              <ul className="grid gap-2.5">
                {stockRequests.map((p) => <StockRequestCard key={p.id} pr={p} />)}
              </ul>
            );
          }
          return (
...
function StockRequestCard({ pr }: { pr: StockRequest }) {
  const { pending, run } = useApprove();
  const [showReject, setShowReject] = useState(false);
  ...
  return (
    <li className="rounded-md border border-brand-200 bg-brand-50 p-3">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <span className="text-[11.5px] text-ik-ink-3">{pr.lines} {pr.lines === 1 ? "item" : "items"}</span>
      </div>
      <div className="mt-1 text-[12.5px] text-ik-ink-2">
        Raised by {pr.requestedBy}{pr.orderCode ? ` · order ${pr.orderCode}` : ""}
      </div>
      <div className="mt-2.5 flex flex-wrap items-center gap-2">
        <Button size="sm" disabled={pending}
          onClick={() => run(() => approvePurchaseRequisition(pr.id), "Approved")}>Approve</Button>
        <Button size="sm" variant="outline" disabled={pending} onClick={() => setShowReject((v) => !v)}>Reject</Button>
        <Link href={`/procurement/purchase-requisitions/${pr.id}`} className="ml-auto text-[11.5px] text-brand hover:underline">Open</Link>
      </div>
      {showReject && (
        <div className="mt-2 grid gap-2 rounded-md border border-ik-rule bg-ik-card p-2.5">
          <Textarea rows={2} placeholder="Reason for rejecting" value={reason} onChange={(e) => setReason(e.target.value)} />
          <div className="flex gap-2">
            <Button size="sm" variant="outline" disabled={pending || !reason.trim()}
              onClick={() => run(() => rejectPurchaseRequisition(pr.id, reason), "Rejected")}>Confirm reject</Button>
            <Button size="sm" variant="ghost" disabled={pending} onClick={() => setShowReject(false)}>Cancel</Button>
          </div>
        </div>
      )}
