...
export default async function PRDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [pr, session, tiers] = await Promise.all([
    getPurchaseRequisition(id),
    auth(),
    getSettingOr<{ adminMin: number }>("po.approvalTiers", { adminMin: 5000 }),
  ]);
  if (!pr) notFound();
  ...
  // Value-routed sign-off: ≥ adminMin (default ₹5,000) is Admin's call;
  // below it, the Manager (or Admin) can clear it.
  const adminMin = new Decimal(typeof tiers?.adminMin === "number" && tiers.adminMin >= 0 ? tiers.adminMin : 5000);
  const total = pr.lines.reduce(
    (s, l) => s.plus(new Decimal(l.requestedQty.toString()).times(new Decimal(l.unitCostSnapshot.toString()))),
    new Decimal(0),
  );
  const needsAdmin = total.gte(adminMin);
  const canApprove =
    pr.status === PurchaseRequisitionStatus.PENDING_APPROVAL &&
    (role === Role.ADMIN || (role === Role.MANAGER && !needsAdmin));
  const canReject = pr.status === PurchaseRequisitionStatus.PENDING_APPROVAL && isApprover;
    ...
    <>
      ...
      {pr.status === PurchaseRequisitionStatus.DRAFT && (
        <div className="mb-4 rounded-md border border-brand-200 bg-brand-50 p-3 text-[13px] text-ik-ink-2">
          <strong>Next:</strong> Hit <em>Submit</em> when the lines look right. Requests below ₹{adminMin.toFixed(0)}{" "}
          go to the <strong>Manager</strong> for approval; ₹{adminMin.toFixed(0)} and above go to the{" "}
          <strong>Admin</strong>. Once approved it becomes a purchase order directly — no second sign-off.
        </div>
      )}
      {pr.status === PurchaseRequisitionStatus.PENDING_APPROVAL && (
        <div className="mb-4 rounded-md border border-amber bg-amber-wash p-3 text-[13px] text-ik-ink-2">
          <strong>Next:</strong> Waiting on the <strong>{needsAdmin ? "Admin" : "Manager"}</strong> to approve
          or reject this request {needsAdmin ? `(it's ₹${adminMin.toFixed(0)} or more)` : `(under ₹${adminMin.toFixed(0)})`}.
        </div>
      )}
