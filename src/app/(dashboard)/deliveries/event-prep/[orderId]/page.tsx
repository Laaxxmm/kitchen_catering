...
export default async function EventPrepPage({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  const [order, items] = await Promise.all([getEventPrepOrder(orderId), listBanquetItems({ activeOnly: true })]);
  if (!order) notFound();
  ...
  return (
    <>
      <PageHeader
        ...
        description="Choose what this event needs, issue what's in stock, request the rest from the store, then mark it ready."
