...
export function StaffTable({ staff }: { staff: Staff[] }) {
  ...
  function startCreate() {
    ...
    setName(""); setPhone(""); setCategory(MaintenanceCategory.GENERAL); setNotes("");
  }
  function startEdit(s: Staff) {
    setEditing(s);
    setName(s.name); setPhone(s.phone ?? ""); setCategory(s.category); setNotes(s.notes ?? "");
  }
  ...
  function deactivate(id: string) {
    ...
    startTransition(async () => {
      try {
        const res = await deactivateMaintenanceStaff(id);
        if (!res.ok) {
          toast.error(res.error);
          return;
        }
  ...
  function hardDelete(s: Staff) {
    if (
      !confirm(`Permanently delete "${s.name}"? Refused if they have any historical activities.`)
    )
      return;
    startTransition(async () => {
      try {
      ...
      {editing ? (
        <section className="grid gap-3 rounded-2xl border border-ik-rule bg-ik-card shadow-ik-card p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-4">
            ...
            <div className="grid gap-1.5">
              <Label htmlFor="category">Primary skill</Label>
              <select
                id="category"
                value={category}
        ...
        <div><Button onClick={startCreate}>+ Add staff</Button></div>
      )}

      {staff.length === 0 ? (
        <p className="text-[13px] text-ik-ink-3">No staff yet. Add the first one above.</p>
      ) : (
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {staff.map((s) => (
              <TableRow key={s.id}>
                ...
                <TableCell className="font-mono text-[12px]">{s.phone ?? "—"}</TableCell>
                <TableCell className="text-[12.5px] text-ik-ink-2">{s.notes ?? "—"}</TableCell>
                <TableCell>
                  <span className={"text-[11px] " + (s.active ? "text-positive" : "text-ik-ink-3")}>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-1">
                    {s.active ? (
                      <>
                        <Button size="sm" variant="outline" onClick={() => startEdit(s)} disabled={pending}>Edit</Button>
                        <Button size="sm" variant="outline" onClick={() => deactivate(s.id)} disabled={pending}>Deactivate</Button>
                      </>
                    ) : (
                      <Button size="sm" variant="outline" onClick={() => reactivate(s)} disabled={pending}>Reactivate</Button>
                    )}
                    <Button size="sm" variant="destructive" onClick={() => hardDelete(s)} disabled={pending}>Delete</Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
