...
export function ReceiptForm({ items }: { items: Item[] }) {
  ...
  const [lines, setLines] = useState<Line[]>([{ itemId: "", quantity: "", costPerUnit: "" }]);

  function addLine() { setLines((l) => [...l, { itemId: "", quantity: "", costPerUnit: "" }]); }
  ...
  return (
    <div className="grid max-w-4xl gap-4">
      <section className="grid gap-3 rounded-2xl border border-ik-rule bg-ik-card shadow-ik-card p-4">
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="grid gap-1.5">
            ...
            <input id="receivedAt" type="datetime-local" value={receivedAt} onChange={(e) => setReceivedAt(e.target.value)} className="h-9 rounded-md border border-ik-rule bg-ik-card px-2 text-[13px]" />
          </div>
          <div className="grid gap-1.5">
            <Label htmlFor="sourceContact">From (person / source)</Label>
            <Input id="sourceContact" placeholder="e.g. Vendor — Mr Iyer" value={sourceContact} onChange={(e) => setSourceContact(e.target.value)} />
          </div>
          <div className="grid gap-1.5">
            <Label htmlFor="sourceNote">Reference / note</Label>
            <Input id="sourceNote" placeholder="Invoice / GRN ref" value={sourceNote} onChange={(e) => setSourceNote(e.target.value)} />
          </div>
        </div>
      </section>
            ...
            return (
              <div key={i} className="grid items-end gap-2 sm:grid-cols-[1fr,140px,140px,80px]">
                  ...
                  >
                    ...
                    {items.map((it) => (
                      <option key={it.id} value={it.id}>
                        [{CAT_LABEL[it.category].slice(0, 4).toUpperCase()}] {it.name} ({it.unit})
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="grid gap-1.5">
                  ...
                  <Input id={`cost-${i}`} type="number" inputMode="decimal" placeholder="optional" value={line.costPerUnit} onChange={(e) => updateLine(i, { costPerUnit: e.target.value })} />
                </div>
                <div>
      ...
      <div className="flex gap-2">
        <Button onClick={submit} disabled={pending}>{pending ? "Saving…" : "Save receipt"}</Button>
