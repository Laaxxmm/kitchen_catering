...
import { stockHealthRows } from "@/server/reports/stock-health";
 ...
 * The computation is shared with the dashboard and the stores panel — see
 * src/server/reports/stock-health.ts for why there is only one of it.
 */
export async function listStockHealth() {
  await requireRole(READ_ROLES);
  return stockHealthRows();
}

export type { StockHealthRow } from "@/server/reports/stock-health";
