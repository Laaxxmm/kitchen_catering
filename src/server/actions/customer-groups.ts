...
export async function createCustomerGroup(raw: unknown) {
...
export async function updateCustomerGroup(id: string, raw: unknown) {
  const session = await requireRole(WRITE_ROLES);
  const input = CustomerGroupInput.parse(raw);
...
export async function deactivateCustomerGroup(id: string) {
  const session = await requireRole(WRITE_ROLES);
  await db.$transaction(async (tx) => {
